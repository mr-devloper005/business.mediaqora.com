  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-[2rem] border border-white/12 bg-[var(--slot4-surface-bg)] p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.24em] text-white/50">
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                ...
                className="h-11 w-full rounded-2xl border border-[var(--editable-border)] bg-black text-white pl-9 pr-3 text-sm outline-none"
              />
            </div>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-2xl border border-white/12 bg-[var(--slot4-surface-bg)] p-5 shadow-sm">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="font-semibold text-white">{item.name}</p>
                    <p className="mt-1 text-xs text-white/50">{formatDate(item.createdAt)}</p>
                  </div>
                  {item.articleSlug ? (
                ...
                {item.articleTitle ? <p className="mt-4 text-sm font-medium text-white">{item.articleTitle}</p> : null}
                <p className="mt-3 text-sm leading-7 text-white/50">{item.comment}</p>
              </article>
            ))}
          </section>
        ) : (
          <section className="mt-8 rounded-2xl border border-dashed border-white/12 bg-[var(--slot4-surface-bg)] p-8 text-center">
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-white/12 bg-[var(--slot4-surface-bg)] p-4 text-sm text-white/50">
